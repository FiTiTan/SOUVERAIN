...
import { MediathequeImporter } from './MediathequeImporter';
...
interface MediathequeViewProps {
    onBack?: () => void;
}

export const MediathequeView: React.FC<MediathequeViewProps> = ({ onBack }) => {
    ...
    // @ts-ignore
    const { items, loading, error, deleteItem, importFiles, fetchItems } = useMediatheque(activePortfolioId);
    ...
    return (
        <MediathequeImporter activePortfolioId={activePortfolioId} onImportSuccess={() => fetchItems()}>
            <div 
                style={{ 
                    padding: '2rem', 
                    height: '100%', 
